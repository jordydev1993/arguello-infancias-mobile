...
export function ResidentCard({ resident, onPress }: ResidentCardProps) {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`${nombre}, ${edadLabel(resident.fecha_nacimiento)}`}
      onPress={onPress}
      className="flex-row items-center gap-3 rounded-lg border border-line bg-canvas p-4 active:bg-surface">
      {resident.foto_url ? (
        <Image source={{ uri: resident.foto_url }} style={{ width: 48, height: 48, borderRadius: 24 }} />
      ) : (
      ...
      <View className="flex-1 gap-0.5">
        ...
        <Text className="text-body-sm text-ink-secondary">{edadLabel(resident.fecha_nacimiento)}</Text>
      </View>
